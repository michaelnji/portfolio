...
export async function getPost(slug: string | undefined): Promise<PostResponse> {
	const query = `*[_type == "post" && slug.current == "${slug}"]{_id,body, "authorInfo":author-> {slug,name,bio,twitter,"imageUrl":image.asset->url}, "tags":categories[]->{title,description}, publishedAt,excerpt, slug, title, _updatedAt,"imageUrl": mainImage.asset->url, "tableOfContents": content[style in ["h2", "h3"]]}`;
	try {
		const data = await sanityClient.fetch(query);
